...
def tsne_bh_prob_graph_nn(X, perplexity, metric, labels=None):
    size = len(X)
    ...
    # remove edges with very low probability
    overall_neighbors = len(X) * perplexity * 2 #times 2 bc symmetric so every value is there twice (could also be solved by np.triu? TODO)
    top_x_indices = np.argsort(data)[::-1][:overall_neighbors]
    filtered_data = data[top_x_indices]
    ...
    for i in top_x_indices:
        g.add_edge(row_idx[i], col_idx[i], weight=data[i] / np.sum(filtered_data))
